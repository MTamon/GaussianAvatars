...
Phase B:  残り全テイクを `--load_globals_only --freeze_globals_from_init`
...
があります (`load_globals_only` / `freeze_globals_from_init` フラグ)。本リポジトリの
...
| `--sequence-file` | (必須・複数指定) | 入力動画(`data/monocular/` 配下) |
